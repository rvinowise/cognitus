...
std::vector<Circuit> CircuitFinder::find_circuits_backward(const Bend& last_end_bend)
{
    ...
    for (--i_prev_bend;
        ...
        //find_parallel_line_of_same_nodes_for_this(prev_bend, last_end_bend);
        ...
        for (std::size_t i_brother_bend = last_end_bend.get_index_in_master_node()-1;
            ...
            i_prev_brother_bend.end_loop_after_this_bend(
                        prev_brother_bend.get_prev_brother_bend());
            for (--i_prev_brother_bend;
                 i_prev_brother_bend != Iterator_bend_BFS::end();
                 --i_prev_brother_bend) {
                Bend prev_brother_bend = *i_prev_brother_bend;
                
                if (prev_bend == prev_brother_bend) {
                    //create_circuit(prev_bend, last_end_bend, 
                                   //prev_brother_bend, brother_bend);
                }
            }
        }
